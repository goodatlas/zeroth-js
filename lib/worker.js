...
class Socket {
  constructor(params) {
    ...
    const { language, finalOnly } = config.defaultParams;
    this.params = {
      key: params.key,
  ...
  connect = () => {
    const { serverAddr, serverPort, sampleRate } = config;
    const { key, language, finalOnly } = this.params;
    ...
    const uri = `ws://${serverAddr}:${serverPort}/client/ws/speech?${query}`;
    debug('uri', uri);
