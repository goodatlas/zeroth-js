...
export default class ZerothMic extends ZerothBase {
  ...
  resample = (audioBuffer, targetSampleRate, onComplete) => {
    ...
    const samples = audioBuffer.length * (window.OfflineAudioContext ? targetSampleRate : 44100) / audioBuffer.sampleRate;
    const offlineContext = new (window.OfflineAudioContext || window.webkitOfflineAudioContext)(channels, samples, window.OfflineAudioContext ? this.sampleRate : 44100);
